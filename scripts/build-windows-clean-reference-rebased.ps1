...
setlocal
cd /d "%~dp0"
echo Starting DarkWolf RTCW DXR Clean Visible Reference build...
WolfSP.exe +set r_dxr 1 +set r_dxrFallbackLight 1 +set r_dxrFallbackLightRadius 900 +set r_dxrFallbackLightIntensity 6 +set r_dxrAmbientIntensity 1.35 +set r_dxrLegacyBlend 0.65 +set r_dxrExposure 1.15 +set r_dxrShadowBias 0.02 +set r_dxrDebug 1 +set r_dxrDebugMode 0
'@
    Set-Content -LiteralPath (Join-Path $dist 'RUN_DARKWOLF_DXR_CLEAN_REFERENCE.bat') -Value $launcher -Encoding ASCII

    $readme = @'
DarkWolf RTCW DXR Clean Visible Reference artifact
========================================
...
Optional: run RUN_DARKWOLF_DXR_CLEAN_REFERENCE.bat after copying the files.
