...
try {
    ...
    foreach ($cfg in @(
        'dxr_stable_clear_v21.cfg',
        'dxr_stable_clear_fast_v21.cfg',
        'dxr_stable_real_lights_v21.cfg',
        'dxr_stable_reference_quality_v21.cfg',
        'dxr_stable_shadow_mask_v21.cfg'
    )) {
    ...
    $launchers = [ordered]@{
        'RUN_DXR_STABLE_CLEAR_V21.bat'             = 'dxr_stable_clear_v21.cfg'
        'RUN_DXR_STABLE_CLEAR_FAST_V21.bat'        = 'dxr_stable_clear_fast_v21.cfg'
        'RUN_DXR_STABLE_REAL_LIGHTS_V21.bat'       = 'dxr_stable_real_lights_v21.cfg'
        'RUN_DXR_REFERENCE_QUALITY_V21.bat'         = 'dxr_stable_reference_quality_v21.cfg'
        'RUN_DXR_SHADOW_MASK_SAFE_V21.bat'          = 'dxr_stable_shadow_mask_v21.cfg'
    }

    foreach ($entry in $launchers.GetEnumerator()) {
        $launcher = "@echo off`r`nsetlocal`r`ncd /d `"%~dp0`"`r`necho Starting DarkWolf RTCW DXR Stable Clear v2.1: $($entry.Value)`r`nWolfSP.exe +set r_picmip 0 +set r_picmip2 0 +set r_roundImagesDown 0 +set r_simpleMipMaps 0 +set r_texturebits 32 +set r_textureMode GL_LINEAR_MIPMAP_LINEAR +exec $($entry.Value)`r`n"
...
This artifact is built from the proven Clean Visible Reference plus the
Stable Clear v2.1 correction. It does not contain RTCW pk3 game data.

Start with:
  RUN_DXR_STABLE_CLEAR_V21.bat

This primary profile uses the proven synchronous path, deterministic one-ray
shadows, AO/sky sampling disabled, no contact shadows, no opaque alpha-cutout
geometry, and full-resolution source texture cvars.

Only after the primary profile is stable, optionally test:
  RUN_DXR_STABLE_CLEAR_FAST_V21.bat
    Removes the global glFinish but keeps ordered DXR fences. Experimental.

  RUN_DXR_STABLE_REAL_LIGHTS_V21.bat
    Disables the camera fallback light to inspect authored map/torch lights.

  RUN_DXR_REFERENCE_QUALITY_V21.bat
    Slow comparison with the old 12/24/8 sample budget. Not a gameplay target.

  RUN_DXR_SHADOW_MASK_SAFE_V21.bat
    Diagnostic grayscale mask. White/gray output is expected.

Do not use the old v2 BALANCED/PERFORMANCE/QUALITY BAT files with this build.
They requested the unsafe async path and low-sample stochastic AO/sky.
...
The log marker should begin with:
  DXR v2.1 stable-clear:
'@
    Set-Content -LiteralPath (Join-Path $dist 'README_RUN_DXR_STABLE_CLEAR_V21.txt') -Value $readme -Encoding UTF8

    Write-Host 'Stable Clear v2.1 runtime package prepared in dist:'
