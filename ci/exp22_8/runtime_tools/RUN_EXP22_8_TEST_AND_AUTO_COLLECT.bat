...
set "ROOT=%CD%"
set "SCRIPT=%ROOT%\EXP22_8_TEST_AND_AUTO_COLLECT.ps1"
set "RESULTS=%ROOT%\EXP22_8_RESULTS"
set "LASTERROR=%RESULTS%\EXP22_8_COLLECTOR_LAST_ERROR.txt"
...
if not exist "%SCRIPT%" (
  echo.
  echo ERROR: EXP22_8_TEST_AND_AUTO_COLLECT.ps1 was not found.
...
echo ==============================================================
echo  DarkWolf EXP22.8 automatic test and result collector
echo ==============================================================
echo Game root : %ROOT%
...
"%PSEXE%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT%" -Profile "darkwolf_exp22_8_production.cfg"
set "RC=%ERRORLEVEL%"

if not "%RC%"=="0" (
  echo.
  echo ERROR: EXP22.8 collector exited with code %RC%.
  if exist "%LASTERROR%" (
    echo.
...
echo.
echo EXP22.8 test and collection completed successfully.
exit /b 0
