[CmdletBinding()]
param(
    [string]$Profile = 'darkwolf_exp22_8_production.cfg',
    [switch]$KeepUnpacked,
    [switch]$NoExplorer
)
...
$ResultsRoot = Join-Path $Root 'EXP22_8_RESULTS'
New-Item -ItemType Directory -Path $ResultsRoot -Force | Out-Null

$LastErrorPath = Join-Path $ResultsRoot 'EXP22_8_COLLECTOR_LAST_ERROR.txt'
if (Test-Path -LiteralPath $LastErrorPath) {
    Remove-Item -LiteralPath $LastErrorPath -Force -ErrorAction SilentlyContinue
}

$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$RunName = "DarkWolf-EXP22.8-AutoTest-$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $ProfilePath -PathType Leaf)) {
        $fallback = Get-ChildItem -LiteralPath $MainDir -File -Filter '*exp22_8*.cfg' -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($fallback) {
            $ProfilePath = $fallback.FullName
            $Profile = $fallback.Name
        }
        else {
            throw "EXP22.8 profile was not found: $ProfilePath"
        }
    }
    ...
    foreach ($name in @('BUILD_MANIFEST.txt','SHA256SUMS.txt','FULL_RELEASE_CONTENTS.txt','README_EXP22_8_PERSISTENT_DYNAMIC_SBT_RU.txt')) {
    ...
    Write-Host '=============================================================='
    Write-Host ' DarkWolf EXP22.8 automatic test and result collector v3'
    Write-Host '=============================================================='
    ...
    try {
        ...
        $perfLines = @($allLines | Where-Object { [string]$_ -match 'EXP22_8_PERF\s+' })
        Write-Text -Path (Join-Path $RunDir 'EXP22_8_PERF_RAW.txt') -Text ((($perfLines -join "`r`n") + "`r`n"))
        ...
        $perfCsv = Join-Path $RunDir 'EXP22_8_PERF.csv'
        if ($rows.Count -gt 0) {
        ...
        $fields = @(
            ...
            'sbtCapacity','sbtHighWater','sbtActiveHighWater','sbtLiveRecords','tlasMs','blasMs',
        ...
        Write-Text -Path (Join-Path $RunDir 'EXP22_8_PERF_SUMMARY.csv') -Text (($statLines -join "`r`n") + "`r`n")
        ...
        if ($lowRows.Count -gt 0) {
            $lowRows | Export-Csv -LiteralPath (Join-Path $RunDir 'EXP22_8_LOW_FPS_SAMPLES.csv') -NoTypeInformation -Encoding UTF8
        }
        else {
            Write-Text -Path (Join-Path $RunDir 'EXP22_8_LOW_FPS_SAMPLES.csv') -Text "Sample`r`n"
        }
        ...
        if ($transitions.Count -gt 0) {
            $transitions | Export-Csv -LiteralPath (Join-Path $RunDir 'EXP22_8_TRANSITIONS.csv') -NoTypeInformation -Encoding UTF8
        }
        else {
            Write-Text -Path (Join-Path $RunDir 'EXP22_8_TRANSITIONS.csv') -Text "Sample`r`n"
        }
        ...
        $summary = @()
        $summary += 'DARKWOLF EXP22.8 AUTOMATIC TEST RESULT'
        $summary += '======================================='
        $summary += "CollectorVersion=EXP22.8-v1"
        ...
        if ($rows.Count -lt 10) {
            $summary += 'ATTENTION: fewer than 10 EXP22_8_PERF samples were captured.'
        }
        else {
            $summary += 'PASS: EXP22_8_PERF telemetry was captured.'
        }
