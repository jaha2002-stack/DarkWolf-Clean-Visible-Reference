[CmdletBinding()]
param(
    [string]$Profile = 'darkwolf_exp22_9_production.cfg',
    [switch]$KeepUnpacked,
    [switch]$NoExplorer
)
...
$ResultsRoot = Join-Path $Root 'EXP22_9_RESULTS'
New-Item -ItemType Directory -Path $ResultsRoot -Force | Out-Null

$LastErrorPath = Join-Path $ResultsRoot 'EXP22_9_COLLECTOR_LAST_ERROR.txt'
if (Test-Path -LiteralPath $LastErrorPath) {
    Remove-Item -LiteralPath $LastErrorPath -Force -ErrorAction SilentlyContinue
}

$Stamp = Get-Date -Format 'yyyyMMdd_HHmmss'
$RunName = "DarkWolf-EXP22.9-AutoTest-$Stamp"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $ProfilePath -PathType Leaf)) {
        $fallback = Get-ChildItem -LiteralPath $MainDir -File -Filter '*exp22_9*.cfg' -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($fallback) {
            $ProfilePath = $fallback.FullName
            $Profile = $fallback.Name
        }
        else {
            throw "EXP22.9 profile was not found: $ProfilePath"
        }
    }

    # Back up and remove old engine console logs so this run is isolated.
    $oldLogs = @()
    ...
    foreach ($file in @(Get-ChildItem -LiteralPath $Root -Recurse -File -ErrorAction SilentlyContinue |
        Where-Object {
            ...
            $_.Name -match '^(qconsole|rtcwconsole).*\.log$'
        })) {
    ...
    $keyFiles = @(
        'WolfSP.exe','OpenAL32.dll','dxcompiler.dll','dxil.dll','sl.interposer.dll',
        'main/cgamex64.dll','main/qagamex64.dll','main/uix64.dll',('main/' + $Profile)
    )
    $hashLines = @()
    ...
    try {
        foreach ($cpu in @(Get-CimInstance Win32_Processor)) {
            $hardware += "CPU=$($cpu.Name) Cores=$($cpu.NumberOfCores) Threads=$($cpu.NumberOfLogicalProcessors)"
        }
    } catch { $hardware += "CPU_QUERY_ERROR=$($_.Exception.Message)" }
    try {
        foreach ($gpu in @(Get-CimInstance Win32_VideoController)) {
            $hardware += "GPU=$($gpu.Name) Driver=$($gpu.DriverVersion) VRAMBytes=$($gpu.AdapterRAM)"
        }
    } catch { $hardware += "GPU_QUERY_ERROR=$($_.Exception.Message)" }
    ...
    foreach ($name in @('BUILD_MANIFEST.txt','SHA256SUMS.txt','FULL_RELEASE_CONTENTS.txt','README_EXP22_9_PERSISTENT_DYNAMIC_SBT_RU.txt')) {
    ...
    Write-Host '=============================================================='
    Write-Host ' DarkWolf EXP22.9 automatic test and result collector v3'
    Write-Host '=============================================================='
    ...
    try {
        ...
        # Collect both RTCW console naming conventions. The previous collector
        # searched only qconsole*.log while this build writes rtcwconsole.log.
        $logPaths = @()
        $seenLogs = @{}
        foreach ($candidate in @(
            (Join-Path $MainDir 'qconsole.log'),
        ...
        # Screenshots and crash artifacts produced during this run.
        $shotIndex = 0
        foreach ($file in @(Get-ChildItem -LiteralPath $Root -Recurse -File -ErrorAction SilentlyContinue |
            Where-Object {
                $_.FullName -notlike "$ResultsRoot*" -and
        ...
        $perfLines = @($allLines | Where-Object { [string]$_ -match 'EXP22_9_PERF\s+' })
        Write-Text -Path (Join-Path $RunDir 'EXP22_9_PERF_RAW.txt') -Text ((($perfLines -join "`r`n") + "`r`n"))
        ...
        foreach ($line in $perfLines) {
            ...
            foreach ($match in [regex]::Matches([string]$line, '(?<key>[A-Za-z][A-Za-z0-9_]*)=(?<value>-?[0-9]+(?:\.[0-9]+)?)')) {
                $number = 0.0
                if ([double]::TryParse($match.Groups['value'].Value, [Globalization.NumberStyles]::Float, $Invariant, [ref]$number)) {
                    $record[$match.Groups['key'].Value] = $number
                }
            }
            $rows += [pscustomobject]$record
        }

        $perfCsv = Join-Path $RunDir 'EXP22_9_PERF.csv'
        if ($rows.Count -gt 0) {
            $rows | Export-Csv -LiteralPath $perfCsv -NoTypeInformation -Encoding UTF8
        }
        else {
            Write-Text -Path $perfCsv -Text "Sample`r`n"
        }
        ...
        $fields = @(
            ...
            'fullRebuildReason','highWaterGrowthEvents','newDynamicSlots','transientSkipped','muzzleInstances',
            'sbtCapacity','sbtHighWater','sbtActiveHighWater','sbtLiveRecords','tlasMs','blasMs','blasBudget','blasBuildNow','blasUpdateNow','blasDeferredNow','blasTrianglesNow','blasBatches','blasBuilds','blasUpdates','blasDeferred','blasTriangles','blasFenceWaitMs','blasSubmitMs','viewWeaponSkipped',
        ...
        foreach ($stat in $stats) {
            $statLines += ("{0},{1},{2},{3},{4},{5},{6},{7}" -f
                $stat.Name,$stat.Count,(Fmt $stat.First),(Fmt $stat.Last),
                (Fmt $stat.Min),(Fmt $stat.Max),(Fmt $stat.Avg),(Fmt $stat.Delta))
        }
        Write-Text -Path (Join-Path $RunDir 'EXP22_9_PERF_SUMMARY.csv') -Text (($statLines -join "`r`n") + "`r`n")

        $lowRows = @()
        foreach ($row in $rows) {
            $avg = Get-Value -Object $row -Name 'avgFps'
            $low1 = Get-Value -Object $row -Name 'low1Fps'
            if (($null -ne $avg -and $avg -lt 60.0) -or ($null -ne $low1 -and $low1 -lt 30.0)) {
                $lowRows += $row
            }
        }
        if ($lowRows.Count -gt 0) {
            $lowRows | Export-Csv -LiteralPath (Join-Path $RunDir 'EXP22_9_LOW_FPS_SAMPLES.csv') -NoTypeInformation -Encoding UTF8
        }
        else {
            Write-Text -Path (Join-Path $RunDir 'EXP22_9_LOW_FPS_SAMPLES.csv') -Text "Sample`r`n"
        }

        # Counter deltas make shooting-related rebuild spikes easier to identify.
        $transitions = @()
        for ($i = 1; $i -lt $rows.Count; $i++) {
            ...
            $transition = [ordered]@{
                ...
                fullRebuildReason = Get-Value $current 'fullRebuildReason'
                sbtCapacity = Get-Value $current 'sbtCapacity'
                sbtHighWater = Get-Value $current 'sbtHighWater'
                sbtActiveHighWater = Get-Value $current 'sbtActiveHighWater'
                sbtLiveRecords = Get-Value $current 'sbtLiveRecords'
                tlasMs = Get-Value $current 'tlasMs'
                blasMs = Get-Value $current 'blasMs'
            }
            foreach ($counter in @('fullRebuilds','partialUpdates','updatedRecords','preparedRecords','bindingRev','sceneRev','creates','updates','deletes','active','dynamicActive','reusedSlots','highWaterGrowthEvents','newDynamicSlots','transientSkipped','muzzleInstances','sbtHighWater','sbtActiveHighWater','sbtLiveRecords','vramUsedMB')) {
                ...
                if ($null -ne $now -and $null -ne $before) {
                    $transition[$deltaName] = $now - $before
                }
            }
            $transitions += [pscustomobject]$transition
        }
        if ($transitions.Count -gt 0) {
            $transitions | Export-Csv -LiteralPath (Join-Path $RunDir 'EXP22_9_TRANSITIONS.csv') -NoTypeInformation -Encoding UTF8
        }
        else {
            Write-Text -Path (Join-Path $RunDir 'EXP22_9_TRANSITIONS.csv') -Text "Sample`r`n"
        }

        $issues = @($allLines | Where-Object {
            [string]$_ -match '(?i)(R_AddMDCSurfaces|DXGI_ERROR|device removed|device hung|fatal error|D3D12.*(error|failed)|^ERROR:|\berror\b)'
        })
        ...
        $summary = @()
        $summary += 'DARKWOLF EXP22.9 AUTOMATIC TEST RESULT'
        $summary += '======================================='
        $summary += "CollectorVersion=EXP22.9-v1"
        ...
        foreach ($stat in $stats) {
            $summary += ("{0}: first={1} last={2} min={3} max={4} avg={5} delta={6}" -f
                $stat.Name,(Fmt $stat.First),(Fmt $stat.Last),(Fmt $stat.Min),
                (Fmt $stat.Max),(Fmt $stat.Avg),(Fmt $stat.Delta))
        }

        $summary += ''
        $summary += 'AUTOMATIC FLAGS:'
        if ($rows.Count -lt 10) {
            $summary += 'ATTENTION: fewer than 10 EXP22_9_PERF samples were captured.'
        }
        else {
            $summary += 'PASS: EXP22_9_PERF telemetry was captured.'
        }
        if ($collectedLogs.Count -eq 0) {
            $summary += 'ATTENTION: no engine console log was found. Confirm logfile 2 and inspect the game directory for rtcwconsole.log.'
        }
        if ($issues.Count -gt 0) {
            $summary += 'ATTENTION: error-like log lines were detected. Inspect ERRORS_AND_WARNINGS_EXTRACT.txt.'
        }
        else {
            $summary += 'PASS: no D3D12/DXGI/device-removal error-like line was detected.'
        }
        if ($rows.Count -gt 0) {
            ...
            $tableBuiltValues = @()
            foreach ($row in $rows) {
                $value = Get-Value $row 'tableBuilt'
                if ($null -ne $value) { $tableBuiltValues += [double]$value }
            }
            ...
            $firstFull = Get-Value $firstRow 'fullRebuilds'
            $lastFull = Get-Value $lastRow 'fullRebuilds'
            $firstHigh = Get-Value $firstRow 'highWaterGrowthEvents'
            $lastHigh = Get-Value $lastRow 'highWaterGrowthEvents'
            $firstMuzzle = Get-Value $firstRow 'muzzleInstances'
            $lastMuzzle = Get-Value $lastRow 'muzzleInstances'
            if ($null -ne $firstFull -and $null -ne $lastFull) {
                $summary += "FullRebuildDelta=$($lastFull - $firstFull)"
            }
            if ($null -ne $firstHigh -and $null -ne $lastHigh) {
                $summary += "HighWaterGrowthDelta=$($lastHigh - $firstHigh)"
            }
            if ($null -ne $firstMuzzle -and $null -ne $lastMuzzle) {
                $summary += "MuzzleIsolationDelta=$($lastMuzzle - $firstMuzzle)"
            }
            if ($null -ne $firstFull -and $null -ne $lastFull -and
                $null -ne $firstHigh -and $null -ne $lastHigh -and
                ($lastHigh - $firstHigh) -gt 0 -and ($lastFull - $firstFull) -eq 0) {
                $summary += 'PASS: SBT high-water grew without a full resource rebuild.'
            }
            elseif ($null -ne $firstFull -and $null -ne $lastFull -and ($lastFull - $firstFull) -gt 0) {
                $summary += 'ATTENTION: fullRebuilds increased. Correlate dFullRebuilds with fullRebuildReason.'
            }
        }
    ...
    # Archive creation is isolated from parsing. Even when CSV generation fails,
    # the raw logs and error report are still packaged.
    try {
        ...
        try {
            ...
            $fallbackText = Get-ErrorText -ErrorRecord $_
            $CollectorErrors += $fallbackText
            Write-Text -Path $LastErrorPath -Text (($CollectorErrors -join "`r`n---`r`n") + "`r`n")
        }
    }
    ...
    if ($ZipCreated) {
        ...
        Write-Host "Archive creation failed. Unpacked results remain in: $RunDir"
    }
}
