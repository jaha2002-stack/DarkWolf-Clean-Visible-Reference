...
if (-not ('BloomExp181NativeKeys' -as [type])) {
    Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class BloomExp181NativeKeys
{
...
$resultDir = Join-Path $resultRoot ("BLOOM_EXP18_1_{0}" -f $stamp)
...
$modeByVk = @{
    ...
    0x76 = [pscustomobject]@{ Key='F7';  Mode=1; Name='BRIGHT_PASS_MASK' }
    0x77 = [pscustomobject]@{ Key='F8';  Mode=2; Name='BLOOM_ONLY' }
    0x78 = [pscustomobject]@{ Key='F9';  Mode=3; Name='SUBTLE' }
    0x79 = [pscustomobject]@{ Key='F10'; Mode=4; Name='TORCH_LAMPS' }
    0x7A = [pscustomobject]@{ Key='F11'; Mode=5; Name='BALANCED' }
...
Write-Host 'Starting Bloom Visual Lab 18.1 FINAL SCENE.'
Write-Host 'F7 Bright Pass Mask and F8 Bloom Only are mandatory diagnostics.'
$started = Get-Date
...
while (-not $process.HasExited) {
    foreach ($vk in @($modeByVk.Keys | Sort-Object)) {
        $down = (([BloomExp181NativeKeys]::GetAsyncKeyState([int]$vk) -band 0x8000) -ne 0)
...
$modeRecords = @([regex]::Matches($consoleText, 'BLOOM_EXP18_1 mode=\d+[^\r\n]*') | ForEach-Object { $_.Value })
$modeSummary = if ($modeRecords.Count -gt 0) { $modeRecords -join [Environment]::NewLine } else { 'No BLOOM_EXP18_1 mode records found.' }
...
$summary = @"
DarkWolf Bloom Visual Lab 18.1 - FINAL SCENE BLOOM
...
BloomStage=After transparent 3D effects and before 2D HUD
BloomResolution=Quarter resolution, separable horizontal/vertical Gaussian blur
...
F7=BRIGHT_PASS_MASK diagnostic; fire, lamps, muzzle flashes and explosions must be visible
F8=BLOOM_ONLY diagnostic; only blurred glow must be visible
F9=SUBTLE high-threshold gameplay profile
F10=TORCH_LAMPS lower threshold and wider glow
F11=BALANCED production candidate
F12=CINEMATIC_STRONG obvious stress-test
...
Acceptance rules:
- F7 and F8 must be obviously different from F6 and from each other.
- The F7 mask should contain the flame/flash itself, proving transparent effects are included.
- F9-F12 should change halo width and intensity, not merely total screen brightness.
- Reject square halos, full-screen haze, HUD glow, texture blur, flicker or severe FPS loss.
"@
Set-Content -LiteralPath (Join-Path $resultDir 'BLOOM_EXP18_1_SUMMARY.txt') -Value $summary -Encoding UTF8
